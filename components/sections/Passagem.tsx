...
export function Passagem() {
  ...
  return (
    <section id="passagem" ref={wrapperRef} className="relative py-20 md:h-[380vh] md:py-0">
      <div className="md:sticky md:top-0 md:flex md:h-screen md:items-center">
        <Container className="w-full">
          <div className="grid items-center gap-6 md:grid-cols-[minmax(520px,1.2fr)_0.8fr] md:gap-8">
            <div className="min-h-[320px] rounded-3xl border border-white/12 bg-white/[0.03] p-6 sm:p-10">
              <div key={activeStep.title} className="space-y-5 animate-stage-enter" style={{ ["--stage-from" as string]: direction > 0 ? "64px" : "-64px" }}>
                ...
                <p className="max-w-xl text-base leading-relaxed text-white/78 sm:text-xl">{activeStep.description}</p>
              </div>
            </div>

            <aside className="rounded-3xl border border-white/12 bg-slate-950/65 p-6 sm:p-8">
              <div
                ...
                {steps.map((step, index) => {
                  ...
                  return (
                    <button
                      ...
                      className="cursor-pointer rounded-full border px-3 py-1.5 text-xs font-medium uppercase tracking-[0.15em] transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-300"
                      style={{
